...
def generate_comment_text(show_data, filename, timestamps=None):
    ...
    # Links
    raw_links = meta.get('extra_notes_links')
    if raw_links is None:
        links = []
    else:
        links = list(raw_links)
    
    # --- ADDED: Show specific URL ---
    show_url = f"https://www.wefunkradio.com/show/{show_data['show_id']}"
    if show_url not in links:
        # Inserting at the beginning so it's the first link seen
        links.insert(0, show_url)
    
    # Always include PayPal link
    paypal_link = "https://www.paypal.com/ncp/payment/HXP5U7YGTMLT8"
    if paypal_link not in links:
        links.append(paypal_link)
...
def main():
    ...
    with open(log_filename, 'w', encoding='utf-8') as log_file:
        log_action(log_file, "--- WEFUNK ID3 Tagger [FAST MODE - No Cover Art] ---")
